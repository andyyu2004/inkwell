...
use llvm_sys::core::{
    LLVMAddFunction, LLVMAddGlobal, LLVMAddGlobalInAddressSpace, LLVMAddNamedMetadataOperand, LLVMCloneModule, LLVMDisposeModule, LLVMDumpModule, LLVMGetFirstFunction, LLVMGetFirstGlobal, LLVMGetLastFunction, LLVMGetLastGlobal, LLVMGetModuleContext, LLVMGetNamedFunction, LLVMGetNamedGlobal, LLVMGetNamedMetadataNumOperands, LLVMGetNamedMetadataOperands, LLVMGetTarget, LLVMGetTypeByName, LLVMPrintModuleToFile, LLVMPrintModuleToString, LLVMSetDataLayout, LLVMSetTarget
};
...
impl<'ctx> Module<'ctx> {
    ...
    pub fn link_in_module(&self, other: Self) -> Result<(), LLVMString> {
        ...
        #[cfg(any(feature = "llvm3-6", feature = "llvm3-7"))]
        {
            ...
            if code == 1 { Err(LLVMString::new(err_string)) } else { Ok(()) }
        }
